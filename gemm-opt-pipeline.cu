...
struct GemmConfig {
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<3, 3, 3>{}, make_layout(make_shape(Int<8>{}, Int<kTileK>{}),
  ...
  using MMA = decltype(make_tiled_mma(
      mma_atom{}, make_layout(Shape<_2, _2, _1>{}),
      make_layout(Shape<_1, _2, _1>{}))); // should obey TiledNumVal{} %
                                          // AtomNumVal{} == Int<0>{}
  ...
  // (32, 32, 16) 是因为MMA中，根据MMA来计算，M=16x2,
  // N=16x2x2，K=16,16就是MMA的基础shape,M和N是经过MMAAtom后的结果
...
template <typename Config> __host__ __device__ int get_swizzle_n(int tiles_n) {
...
template <typename Config> __host__ dim3 get_swizzled_grid(int m, int n) {
...
// global write opt
template <typename Config>
__global__ void gemm_opt_final(void *Dptr, const void *Aptr, const void *Bptr,
                               int m, int n, int k) {
...
int main(int argc, char *argv[]) {
  ...
  cudaMalloc(&Drefptr, sizeof(T) * M * N);
  ...
  if (algo == "compare4096" || algo == "compare") {
    ...
    printf("%8s %12s %12s %14s %14s %8s %12s\n", "SIZE", "final_ms",
           "cublas_ms", "final_TFLOPS", "cublas_TFLOPS", "algo", "achieve");
    ...
    for (int size = compare_start; size <= compare_end; size += compare_step) {
      ...
      CUDA_CHECK(cudaMemset(Drefptr, 0, sizeof(T) * M * N));
      CublasLtGemm<T> cublaslt_gemm;
      cublaslt_gemm.init(Drefptr, Aptr, Bptr, M, N, K);
      ...
      printf("%8d %12.6f %12.6f %14.6f %14.6f %8d %11.2f%%\n", size, final_ms,
             cublaslt_ms, final_tflops, cublaslt_tflops, cublaslt_best_algo,
             final_tflops / cublaslt_tflops * 100);

      CUDA_CHECK(cudaMemset(Drefptr, 0, sizeof(T) * M * N));
      cublaslt_gemm.run_algo(cublaslt_best_algo);
      CUDA_CHECK(cudaDeviceSynchronize());
      gpu_compare(Dptr, Drefptr, M * N);
    }
  } else if (algo == "final") {
